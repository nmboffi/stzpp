...
int main(int argc, char *argv[]) {
    ...
    double zeta = qs? 1 : 1e4;                              // Scaling parameter for comparison between explicit and direct simulations.
    ...
    double U   = 1e-7*zeta;                                 // Magnitude of velocity at time and bottom faces (in units of L/ts)
    double kap = .35;                                       // Viscous damping
    double om = 2*M_PI/(tf-t0);                            // Omega value for friction welding.
    double rsq = 1./9.;                                     // rsq value for friction welding.
    ...
    double diff_l = 1.5*(b_x - a_x)/N_x;                                // Diffusion lengthscale (6 grid points wide)
    E = E/sy;
    mu = mu/sy;
    rho = mu;
    K = K/sy;
    ...
    if (lrank == 0) { printf("tscale: %f, tf/tscale: %f, 15*tscale/ts: %f\n", tscale, tf/tscale, 15*tscale); }
    if (lrank == 0) { printf("tf: %f, frame length: %f, qdt: %f\n", tf, tf/N_tp, tf/N_tp/qs_steps); }

    // Create the Cartesian communicator
    MPI_Cart_create(MPI_COMM_WORLD, ndims, dims, periods, reorder, &cart_comm);
    
    // Create the simulation object
    shear_sim_3d<sym_mat3> sim = shear_sim_3d<sym_mat3>(lN_x, lN_y, lN_z, 
                                                        a_x, a_y, a_z, 
                                                        b_x, b_y, b_z,
                                                        t0, tf, tmult, N_tp, output,
                                                        mu, lamb, rho, qs? 0 : kap, 
                                                        sy, tau0, c0, eps0,
                                                        delta, omega, bath_temp, chi_inf,
                                                        ez, TZ, diff_l, U, gauss_fac, ll, chi_mu, chi_sigma, zeta, 
                                                        &cart_comm, dims, sim_case, om, rsq, periods);
