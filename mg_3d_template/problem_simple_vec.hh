...
class problem_simple {
	public:
        ...
        /* The value of the spatial discretization. */
        const double hh;

        /* The inverse of the spatial discretization. */
        const double ih;

        /* The big man Pi himself. */
		...
		problem_simple(geometry &gm) :
            ...
            // Assume we are solving on the unit cube.
            hh(2./(gm.m-1)), ih((gm.m-1)/2.),
        ...
        * Lower bounds are inclusive, upper bounds are exclusive, hence -1 and +2. */
		...
		inline V r_field(int i, int j, int k) {
            /*
			 *return ((i == 3) && (j == 3) && (k == 3))? V(1) :
             *    (((i == 12) && (j == 12) && (k == 12))? V(-1) : V(0));
             */
//			return k==0&&i>=4&&i<=8&&j>=4&&j<=8?1:0;
            ...
            double cx = cos(pi*x_val);
            double sx = sin(pi*x_val);
            double sy = sin(pi*y_val);
            //return V(-72*pi4*cx*cx*sx*sy + 28*pi4*sx*sx*sx*sy, 0);
            //return V(4*pi4*sx*sy, 0);
            return V(-2*pi*pi*sx*sy);
		}
    ...
    /* Stencil for the system L(u) = f, L(v) = u,
     * where L denotes the Laplacian. */
    M stencil[27] = {M(0),     M(0), M(0),     M(0),                   M(1./hh),     M(0), M(0),     M(0), M(0),
                     M(0), M(1./hh), M(0), M(1./hh),      M(-6/hh, 0, 0, -6/hh), M(1./hh), M(0), M(1./hh), M(0),
                     M(0),     M(0), M(0),     M(0),                   M(1./hh),     M(0), M(0),     M(0), M(0)};
};
...
template <typename V, typename M>
void problem_simple<V, M>::fill_entries(int i, int j, int k, M *&en) {
...
/*
 *template <typename V, typename M>
 *M problem_simple<V, M>::stencil[27] = {M(0), M(0), M(0), M(0), M(1),  M(0), M(0), M(0), M(0),
 *                                       M(0), M(1), M(0), M(1), M(-6), M(1), M(0), M(1), M(0),
 *                                       M(0), M(0), M(0), M(0), M(1),  M(0), M(0), M(0), M(0)};
 */
...
template <typename V, typename M>
M problem_simple<V, M>::fix[1] = {M(-12)};
